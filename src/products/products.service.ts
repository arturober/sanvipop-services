...
export class ProductsService {
    constructor(
        ...
        @InjectRepository(ProductPhoto) private readonly prodPhotoRepository: EntityRepository<ProductPhoto>,
    ...
    private async getAndCheckProduct(authUser: User, id: number, relations: string[] = []): Promise<Product> {
        const product = await this.productRepository.findOne({id}, relations);
        if(!product) {
            throw new NotFoundException('Product not found');
        }
        if(product.owner.id !== authUser.id) {
            throw new ForbiddenException('You can\'t edit or delete other user\'s products');
        }
        return product;
    }

    async findAll(authUser: User): Promise<Product[]> {
        const products = await this.productRepository.findByDistance(0, 0, authUser.id); 
    ...
    async findAllByDistance(authUser: User): Promise<Product[]> {
        const products = await this.productRepository.findByDistance(authUser.lat, authUser.lng, authUser.id); 
        return this.productRepository.populate(products, ['owner', 'mainPhoto', 'category']);
    }
    ...
    async findBookmarked(authUser: User, idUser: number): Promise<Product[]> {
        ...
        const products = await this.productRepository.findByDistance(authUser.lat, authUser.lng, authUser.id, null, undefined, joinBookmark); 
        return this.productRepository.populate(products, ['owner', 'mainPhoto', 'category']);
    }

    async findByOwner(authUser: User, idUser: number): Promise<Product[]> {
        const products = await this.productRepository.findByDistance(authUser.lat, authUser.lng, authUser.id, {owner: {id: idUser}, $not: {status: 3}}); 
    ...
    async findById(authUser: User, id: number): Promise<Product> {
        const product = await this.productRepository.findById(id, authUser.id, authUser.lat, authUser.lng);
        if(!product) {
            throw new NotFoundException('Product not found');
        }
        product.numVisits++;
    ...
    async insert(authUser: User, prodDto: InsertProductDto): Promise<Product> {
        const photoUrl = await this.imageService.saveImage('products', prodDto.mainPhoto);
        ...
        const product = new Product(prodDto.title, prodDto.description, prodDto.price, user, category);
    ...
    async update(authUser: User, id: number, prodDto: EditProductDto): Promise<Product> {
        const product = await this.getAndCheckProduct(authUser, id, ['owner', 'mainPhoto', 'category']);
        ...
        if(prodDto.mainPhoto) {
            product.mainPhoto = await this.prodPhotoRepository.findOne(prodDto.mainPhoto);
    ...
    async buyProduct(authUser: User, id: number): Promise<void> {
        const product = await this.productRepository.findOne({id}, ['owner', 'mainPhoto', 'category']);
        if(!product) {
            throw new NotFoundException('Product not found');
        }
        product.status = 3;
    ...
    async delete(authUser: User, id: number): Promise<void> {
        ...
        await this.productRepository.removeAndFlush(product); // TODO: Usuario logueado
    }

    async addBookmark(authUser: User, idProd: number): Promise<void> {
        const bookmark = await this.prodBookmarkRepository.findOne({$and: [{user: {id : authUser.id}}, {product: {id: idProd}}]});
        if(!bookmark) {
            try {
                await this.prodBookmarkRepository.nativeInsert({idUser: authUser.id, idProduct: idProd});
            } catch(e) {
                throw new NotFoundException('Product not found');
            }
        }
    ...
    async removeBookmark(authUser: User, idProd: number): Promise<void> {
        await this.prodBookmarkRepository.nativeDelete({idUser: authUser.id, idProduct: idProd});
    }

    async addPhoto(authUser: User, idProd: number, photoDto: AddPhotoDto): Promise<ProductPhoto> {
        const product = await this.getAndCheckProduct(authUser, idProd);
        const photoUrl = await this.imageService.saveImage('products', photoDto.photo);
        const photo = new ProductPhoto(photoUrl);
        photo.product = product;
        await this.prodPhotoRepository.persist(photo);
        if(photoDto.setMain) {
    ...
    async removePhoto(authUser: User, idProd: number, idPhoto: number): Promise<void> {
        const product = await this.getAndCheckProduct(authUser, idProd, ['photos']);
        const photo = product.photos.getItems().find(photo => photo.id === idPhoto);
        if(!photo) {
            throw new NotFoundException('Photo not found in this product');
        }
        await this.prodPhotoRepository.removeAndFlush(photo);
    }
}
